...
├── Riwayat Pembayaran — row just below Paket Saya, opens a modal (`showPaymentHistory()`) listing every `token_purchases` row the user owns (not just successful ones — pending/paid/expired/failed all show, each with a color-coded status badge), newest first. Package label is derived from the `tokens` count client-side, not a stored column. Tapping a row (`openPaymentHistoryRow()`): a `pending` one resumes that same payment (`resumePayment()` → `/api/create-payment` with `resume_purchase_id`, redirects to Xendit); any other status shows a read-only detail view (amount, date, payment method if captured) with a "← Kembali" link back to the list
