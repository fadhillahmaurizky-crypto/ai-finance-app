...
└── Settings' own "Ganti Paket" upgrade picker (renderPlanOptions()/requestPlanChange() in settings.js) is unrelated to this orphaned flow, and as of `wangku-spec-token-slider-ganti-paket.md` **no longer** just opens a WhatsApp link unconditionally: target Free applies instantly (direct PATCH, no payment); target Basic/Pro shows a confirmation modal then pays via `/api/create-payment` (`item_type:'plan', restart:true`) — same Xendit path as `buyPlanRenewal()`, but always restarting the period fresh rather than extending. Ultimate was never offered here (renderPlanOptions() only lists free/basic/pro) so the WhatsApp path is now fully unused for tier changes, though the `wa.me` link-building code remains in `requestPlanChange()`'s history/git blame if ever needed again
