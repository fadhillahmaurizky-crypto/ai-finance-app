...
## 4. Fonnte WhatsApp bot — `gs/fonnte.gs` is real now, but unverified

A Llama-based text parser that extracts `jenis/nominal/kategori/akun/prioritas/keterangan` from a WhatsApp message now exists as `gs/fonnte.gs` (committed as of the Part A / routing-restructure merge — earlier docs describing it as an uncommitted planning-stage draft are stale, don't rely on that older claim). It does **not** yet pass the user's real account/category names as context the way this section originally envisioned — its category matching is a fixed keyword list that doesn't line up with the app's actual `DEFAULT_CATEGORIES`, and it has several other known bugs (wrong Supabase key tier, missing `account_id`, month-only balance math). See `backend.md` for the full list before assuming any of this works correctly end-to-end — no dedicated fix pass has happened yet.
