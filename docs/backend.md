...
2. **`POST /api/xendit-webhook`** — Xendit calls this once the invoice is actually paid. This is the **source of truth**, not the client returning from checkout (that redirect can be skipped, delayed, or the URL guessed at by anyone — it proves nothing about whether payment actually happened). Two things happen in order:
   ...
   - **Idempotent grant**: `UPDATE token_purchases SET status='paid' WHERE id=<external_id> AND status='pending'`. Xendit can and does redeliver the same callback (retries) — this `WHERE status='pending'` clause *is* the idempotency guarantee, no separate locking needed. A redelivered callback matches zero rows (the row's already `'paid'` from the first delivery) and the function just acknowledges `200` without granting anything a second time. Only when a row is actually flipped does the function `PATCH users SET tokens_limit=<tier>, tokens_used=0` — **the exact same semantics as `admin.html`'s `setUserTokens()`** (replaces the limit outright, doesn't add on top of whatever was left), so a purchased top-up means the same thing whether an admin grants it manually or a user buys it themselves.

**Client side** (`js/settings.js`): a "Beli Token" mini-section in the Pro-plan card (`renderPlanCard()` in `app-core.js`) replaces what used to be a single "Isi Token Lagi" button that opened WhatsApp with a stale, wrong price (`Rp 29.000`, matching neither real tier) — now two real tier buttons calling `buyTokenPackage()`. After returning from the Xendit-hosted checkout (`?xendit_return=success|failed` on the redirect URL), `checkXenditReturn()` — called once per `showApp()` — shows a "Memproses pembayaran..." toast and polls `get_user_by_id` every 3 seconds (up to 6 times) rather than assuming `tokens_limit` is already updated the instant the user is back, since the webhook can land a few seconds after the redirect.

**Verified without live credentials** (idempotency and grant-semantics logic, directly against the database): inserted a real `'pending'` row, ran the exact `UPDATE ... WHERE status='pending'` the webhook runs — first attempt returns the row and flips it to `'paid'`, an identical second attempt (simulating a Xendit retry) returns zero rows, confirming a token grant can't double-fire. Separately confirmed `token_purchases` is genuinely unreadable via a plain anon-key REST call (`200` + empty array, same as `password_reset_tokens`).

**Not verified — needs `XENDIT_SECRET_KEY_TEST`/`XENDIT_WEBHOOK_VERIFICATION_TOKEN_TEST` actually set in Vercel and a real Xendit sandbox session, neither of which is available from here**: that `create-payment.js`'s real call to Xendit's Invoice API succeeds and returns a working `checkout_url`; that a real Xendit sandbox test payment actually triggers a live webhook delivery; that the full click-through (Settings → pick a tier → pay in sandbox → land back in the app → tokens actually increase) works end to end. See `roadmap.md`.
